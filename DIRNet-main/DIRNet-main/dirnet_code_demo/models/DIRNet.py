...
class ISRResCNet(torch.nn.Module):
    """
    Dynamic Iterative Residual NETWORK (DIRNet).
    """
    ...
    def forward_all_iter(self, p, init, noise_estimation, max_iter=None):
        ...
        xpre = 0
        threshold = 1e-4  # 初始阈值
        improvement_threshold = 1e-3  # 改进阈值
        for i in range(max_iter):
            xcur, xpre = self.forward(xcur, xpre, p, i)
            if i > 0 :
                improvement = torch.norm(xcur - xpre)
                if improvement < threshold:   # 动态调整迭代次数
                    break  # 如果改进非常小，提前终止迭代
                if improvement < improvement_threshold:
                    threshold = improvement  # 动态调整阈值
        return xcur
